...
test('Проверка доступности элементов headera неавторизованного пользователя', async ({
  mainPage,
}) => {
...
test('Проверка доступности элементов tabov категории неавторизованного пользователя', async ({
  mainPage,
}) => {
...
test('Проверка доступности элементов по добавлению контента неавторизованного пользователя', async ({
  mainPage,
}) => {
  await mainPage.clickAddButton();
  await mainPage.addPopupButtonSnapshot();
});
test('Проверка доступности попап элементов в уведомлениях неавторизованного пользователя', async ({
  mainPage,
}) => {
  await mainPage.clickNotificationButton();
  await mainPage.notificationPopupButtonSnapshot();
});
test('Проверка доступности попап элементов в "Безопасный режим" неавторизованного пользователя', async ({
  mainPage,
}) => {
  await mainPage.clickSaveModeButton();
  await mainPage.saveModePopupButtonSnapshot();
});
test('Проверка доступности элементов модального окна в авторизации неавторизованного пользователя', async ({
  mainPage,
}) => {
...
test('Проверка доступности элементов меню навигации неавторизованного пользователя - раскрыт', async ({
  mainPage,
}) => {
